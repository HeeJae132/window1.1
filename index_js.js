...
document.addEventListener('DOMContentLoaded', () => {
  // Clock_Screen과 Text_Screen 요소를 가져와서 처음부터 표시
  const clockScreen = document.getElementById('Clock_Screen'); // Clock_Screen 요소 가져오기
  const textScreen = document.getElementById('Text_Screen'); // Text_Screen 요소 가져오기

  if (clockScreen) {
    clockScreen.style.display = 'block'; // Clock_Screen을 처음부터 표시
  } else {
    console.error("Clock_Screen 요소를 찾을 수 없습니다.");
  }

  if (textScreen) {
    textScreen.style.display = 'block'; // Text_Screen을 처음부터 표시
  } else {
    console.error("Text_Screen 요소를 찾을 수 없습니다.");
  }
});
...
document.querySelector('.start').addEventListener('click', function() {
  this.style.display = 'none'; // 클릭 시 해당 요소 숨기기
});
document.querySelector('.start').addEventListener('click', function() {
  this.classList.add('hidden'); // 클릭 시 .hidden 클래스를 추가하여 서서히 사라지도록 함
});

// closebutton 클릭 시 alert_close를 보이도록
document.querySelector('.closebutton').addEventListener('click', function() {
  const alertClose = document.querySelector('.alert_close');
  alertClose.style.display = 'block';  // alert_close를 보이게 설정
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 초기 확인: 요소가 있는지 확인
  if (!startPage || !closeButton || !alertClose || !yesButton || !noButton) {
    console.error("필요한 요소를 찾을 수 없습니다. HTML 구조를 확인하세요.");
    return;
  }

  // 초기 상태: start 요소는 보이고 alert_close는 숨김
  ...
  // closebutton 클릭 시 alert_close 표시
  closeButton.addEventListener("click", () => {
    alertClose.style.display = "block";
  });

  // 네 버튼 클릭 시 alert_close 숨기고 start 다시 표시
  yesButton.addEventListener("click", () => {
    alertClose.style.display = "none"; // 알림창 숨기기
    startPage.style.display = "block"; // start 다시 표시
  });

  // 아니요 버튼 클릭 시 alert_close 숨기기
